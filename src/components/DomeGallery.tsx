...
const getDataNumber = (el: HTMLElement, name: string, fallback: number) => {
  ...
  const n = attr == null ? NaN : parseFloat(attr as string);
...
function buildItems(pool: SouvenirImage[], seg: number): TileCoord[] {
  ...
  const normalizedImages = pool.map(image => {
    const src = image.src || image.fullUrl || image.thumbnailUrl || '';
    return { src, alt: image.alt || image.displayName || image.caption || '' };
  });
...
export default function DomeGallery({
  ...
  // Auto-rotation animation avec GSAP pour un mouvement très doux
  useEffect(() => {
    if (!sphereRef.current) return;
    
    let animation: gsap.core.Tween;
    const rotationTarget = { y: rotationRef.current.y };
    
    // Animation GSAP pour rotation automatique douce
    const startAutoRotation = () => {
      animation = gsap.to(rotationTarget, {
        y: rotationTarget.y + 360,
        duration: 300, // Très lent : 300 secondes pour un tour complet
        ease: "none",
        repeat: -1,
        onUpdate: () => {
          if (!draggingRef.current && !openingRef.current) {
            rotationRef.current = { x: rotationRef.current.x, y: normalizeAngle(rotationTarget.y) };
